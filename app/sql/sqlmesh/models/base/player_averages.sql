...
WITH games_with_played_flag AS (
  SELECT 
  *,
  ROW_NUMBER() OVER (
      PARTITION BY PLAYER_ID, TEAM_ID, SEASON_ID 
      ORDER BY GAME_DATE
  ) AS GAME_NUMBER,

  CASE WHEN MIN IS NOT NULL
    AND (
...
SELECT
    ...
    AVG(
      CASE 
            WHEN PLAYED_FLAG = 1 THEN CAST(SPLIT_PART(MIN, ':', 1) AS DOUBLE) + CAST(SPLIT_PART(MIN, ':', 2) AS DOUBLE) / 60 
            ELSE NULL 
        END
    ) OVER (
